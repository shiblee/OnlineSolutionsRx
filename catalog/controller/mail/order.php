...
class ControllerMailOrder extends Controller {
	...
	public function edit($order_info, $order_status_id, $comment) {
		...
		$data['text_order_id'] = $language->get('text_order_id');
		$data['text_date_added'] = $language->get('text_date_added').'test';
		$data['text_order_status'] = $language->get('text_order_status').'test';
		$data['text_link'] = $language->get('text_link');
	...
	public function edit_new($order_info, $order_status_id, $comment) {
    $language = new Language($order_info['language_code']);
    $language->load($order_info['language_code']);

    // Load the required language file if needed
    $language->load('mail/order_edit');
    ...
    $data['link'] = $order_info['customer_id'] ? $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_info['order_id'] : '';
    $data['comment'] = strip_tags($comment);

    // Load additional data
    $this->load->model('setting/setting');
    $from = $this->model_setting_setting->getSettingValue('config_email', $order_info['store_id']) ?: $this->config->get('config_email');
    ...
    // Fetch required additional order details for email
    ...
    $data['totals'] = $totals;
    $data['shipping_address'] = $order_info['shipping_address_1'] . ', ' . $order_info['shipping_city'] . ', ' . $order_info['shipping_country'];
    $data['payment_method'] = $order_info['payment_method'];

    // Use status name to conditionally send email template
    $subject = '';
    $template = '';

    switch (strtolower($data['order_status'])) {
        case 'processing':
            $subject = 'Order Status Update - ' . $data['order_id'] . ' from OnlineSolutionsRx';
            $template = 'mail/order_status_processing';
            break;

        case 'complete':
            $subject = 'Your Order ' . $data['order_id'] . ' Has Been Completed - OnlineSolutionsRx';
            $template = 'mail/order_status_completed';
            break;

        
        case 'cancelled':
            $subject = 'Your Order ' . $data['order_id'] . ' Has Been Cancelled - OnlineSolutionsRx';
            $template = 'mail/order_status_cancelled';
            break;
        ...
        default:
            $subject = 'Order Update - ' . $data['order_id'] . ' - OnlineSolutionsRx';
            $template = 'mail/order_edit';
            break;
    }
    // echo $subject;exit();
