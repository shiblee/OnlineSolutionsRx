...
class ControllerProductSpecial extends Controller {
	public function index() {
		...
		$product_total = 16;
		// echo"<pre>";print_r($product_total);exit;

		$results = $this->model_catalog_product->getProductSpecialsCustomSpecial($filter_data);
		...
		foreach ($results as $result) {
			...
			if ((float)$result['special']) {
				...
				// winter infotech countdown start
							$special_date = $this->model_catalog_product->getSpecialDate($result['product_id']);
			...
			if ($this->config->get('config_review_status')) {
				...
				// $tag_per = 0;
				// }
							if ($result['special'] > 0 && $result['special'] != NULL && $result['price'] != 0) {
					    $tag_per = ($result['special'] * 100) / $result['price'];
					    $tag_per = round($tag_per);
			...
			$data['products'][] = array(
				...
				'price'       => $price,
				'special'     => $special,
				// 'end_date'    => $special_date['end_date'],
				'tax'         => $tax,
