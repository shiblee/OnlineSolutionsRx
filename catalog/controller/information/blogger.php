...
class ControllerInformationBlogger extends Controller {
	...
	public function index() {
		...
		$blogger_id = (int)$this->request->get['blogger_id'];
		$blog_info = $this->model_extension_webi_blogger->getBlog($blogger_id);
		...
		// $blogger_cate =$this->model_extension_webi_blogger->getBlogsByModule($blog['module_id']);
		$total_comments = $this->model_extension_webi_blogger->getTotalBlogComments($blogger_id);
		$blogger_cate = $this->model_extension_webi_blogger->getBlogCategoryb($blogger_id);

	$data['topic'] = array(
	    'date_added'        => date('M d, Y', strtotime($blog_info['date_added'])), // Format the date as "Apr 05, 2025"
	    'total_comments'    => number_format($total_comments),
	    'category_name'     => isset($blogger_cate['category_name']) ? $blogger_cate['category_name'] : '',
	    'short_description' => isset($blog_info['short_description']) ? html_entity_decode($blog_info['short_description'], ENT_QUOTES, 'UTF-8') : '',

	    'href'              => $this->url->link('information/blogger', 'blogger_id=' . $blogger_id, true),
	);
...
// echo "<pre>";print_r($module_blogs);exit();
    ...
    $short_desc_raw = isset($module_blog['short_description']) && $module_blog['short_description'] !== null ? $module_blog['short_description'] : '';
    $words = explode(' ', strip_tags($short_desc_raw));
    ...
    $module_blog_data[] = array(
        ...
        'category_name'    => $module_blog['category_name'],
