	...
	public function index() {
		...
		foreach ($categories as $category) {
			...
			if ($category['top']) {
				...
				foreach($children as $child) {
					...
					$children_data[] = array(
						...
						'image_name'  => $this->model_tool_image->resize($child['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_height'))
					);
			...
			$data['categories'][] = array(
				...
				'name'        => $category['name'],
				'description' => utf8_substr(strip_tags(html_entity_decode($category['description'], ENT_QUOTES, 'UTF-8')), 0, 50) . '...',
