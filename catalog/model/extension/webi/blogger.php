...
class ModelExtensionWebiBlogger extends Model {
	...
	public function getBlogsByModule_o($module_id, $limit = 0) {
		// echo "string";exit;
    ...
    $sql = "SELECT 
            ...
            FROM " . DB_PREFIX . "blogger b 
            LEFT JOIN " . DB_PREFIX . "blogger_description bd 
                ON (b.blogger_id = bd.blogger_id) 
            LEFT JOIN " . DB_PREFIX . "blog_category_description cd 
                ON (b.category_id = cd.category_id AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "') 
            WHERE 
                b.status = '1' 
                AND bd.language_id = '" . (int)$this->config->get('config_language_id') . "' 
                AND b.module_id = '" . (int)$module_id . "' 
...
public function getBlogCategoryb_($blogger_id) {
    $query = $this->db->query("SELECT bcd.name FROM " . DB_PREFIX . "blog_category_description btc LEFT JOIN " . DB_PREFIX . "blogger bc ON (bcd.category_id = bc.category_id) LEFT JOIN " . DB_PREFIX . "blog_category_description btc ON (btc.category_id = bc.category_id) WHERE btc.blogger_id = '" . (int)$blogger_id . "' AND bcd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
    
    return $query->row; // or ->rows if blog has multiple categories
}
public function getBlogCategoryb($blogger_id) {
    $query = $this->db->query("SELECT bcd.name AS category_name FROM " . DB_PREFIX . "blog_category_description bcd LEFT JOIN " . DB_PREFIX . "blog_category bc ON (bcd.category_id = bc.category_id) LEFT JOIN " . DB_PREFIX . "blogger btc ON (btc.category_id = bc.category_id) WHERE btc.blogger_id = '" . (int)$blogger_id . "' AND bcd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
    
    return $query->row; // Use ->rows if multiple categories are needed
}
