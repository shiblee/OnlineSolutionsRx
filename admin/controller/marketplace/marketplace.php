...
class ControllerMarketplaceMarketplace extends Controller {
	public function index() {
		...
		// Fetch config values with fallback to empty strings
$opencart_username = $this->config->get('opencart_username') ?? '';
$opencart_secret = $this->config->get('opencart_secret') ?? '';
$host = $this->request->server['HTTP_HOST'] ?? '';
$version = VERSION ?? '';
$time = time(); // Make sure $time is defined
...
// Signature generation with safe string casting
$signature = base64_encode(hash_hmac('sha1', $string, (string)$opencart_secret, true));

// URL build with urlencode/rawurlencode
$url  = '&username=' . urlencode((string)$opencart_username);
$url .= '&domain=' . urlencode((string)$host);
$url .= '&version=' . urlencode((string)$version);
$url .= '&time=' . urlencode((string)$time);
$url .= '&signature=' . rawurlencode((string)$signature);
