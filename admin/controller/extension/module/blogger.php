...
class ControllerExtensionModuleBlogger extends Controller {
	...
	protected function validate() {
		...
		if ((utf8_strlen($this->request->post['name']) < 3) || (utf8_strlen($this->request->post['name']) > 200)) {
	...
	public function add() {
		...
		$this->load->language('extension/module/blogger');
		  // $this->load->model('catalog/category');  // Load the category model
	...
	protected function getForm() {
		...
		$data['heading_title'] = $this->language->get('heading_title');
		  // Get the blogger info if editing
    if (isset($this->request->get['blogger_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
        $blogger_cate = $this->model_extension_webi_blogger->getBlog($this->request->get['blogger_id']);
        $blogger_info = $this->model_extension_webi_blogger->getBlogTitle($this->request->get['blogger_id']);
    }

    // print_r($blogger_info);exit();

    // Pass the Meta fields to the view
    if (isset($blogger_info)) {
        // Set meta title, description, and keywords if available
        $data['meta_title'] = $blogger_info['meta_title'];
        $data['meta_description'] = $blogger_info['meta_description'];
        $data['meta_keyword'] = $blogger_info['meta_keyword'];
    } else {
     ...
     if (isset($blogger_cate)) {
        // Set meta title, description, and keywords if available
        $data['category_id'] = $blogger_cate['category_id'];
    ...
    // // Prepare categories data to pass to the view
    ...
    $this->load->model('extension/module/blog_category');
    $data['categories'] = $this->model_extension_module_blog_category->getCategoriesblog();
